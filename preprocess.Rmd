...
annot_general <- DTU::get_annot("86", gtf_path = gtfPath, strip_version = F)
...
saveRDS(info_rna, "../../rna_protein_coupling_PD/rdsData/info_rna.rds")
info %>% dplyr::select(sample_id, batch, channel, reporter.intensity.id, RNAseq_id, cohort, age_years, sex, PMI, condition1) %>%
	dplyr::mutate(condition1 = case_when(
					     condition1 == "Medium" ~ "other",
					     cohort == "Netherlands Brain Bank" ~ "other",
					     TRUE ~ condition1)) %>%
saveRDS(., "../../rna_protein_coupling_PD/rdsData/info.rds")					     
```
